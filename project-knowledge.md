...
- **PostgreSQL**: Durable source of truth for all business data.
...
1. **PostgreSQL is the durable source of truth.** Redis is only used for temporary or ephemeral coordination. Redis does not decide whether a ride was won; PostgreSQL does.
2. **Atomic Ride Assignment.** A ride can only be won by exactly one captain. A strict `UPDATE ... WHERE captain_id IS NULL` is required to prevent read-then-write race conditions.
...
6. **No GPS Flood to PostgreSQL.** Do not log every live GPS location event to PostgreSQL. Only keep it in Redis and broadcast it via WebSocket.
